...
\begin{enumerate}
  ...
  \item \textbf{Sigui $f \in\mathcal{C}^k$ una funció $2\pi$-periòdica. Demostreu que $\widehat{f}(n)=\O(\abs{n}^{-k})$ quan $\abs{n}\to\infty$ (i.e. existeix una constant $C > 0$ tal que $\abs{\widehat{f}(n)}\leq C\abs{n}^{-k}$).}
